...
'''
'''
SECOND TIME THROUGH---SECOND TIME THROUGH---SECOND TIME THROUGH---SECOND TIME THROUGH---SECOND TIME THROUGH---SECOND TIME THROUGH---SECOND TIME THROUGH---SECOND TIME THROUGH---
'''
...
class Solution:
    def maxProfit(self, prices: List[int]) -> int:
        ...
        for x, num in enumerate(prices):
            if min_num == None or num < min_num:
                min_num = num
            if num - min_num > max_profit:
                max_profit = num - min_num
...
Success
Details 
Runtime: 1391 ms, faster than 68.48% of Python3 online submissions for Best Time to Buy and Sell Stock.
